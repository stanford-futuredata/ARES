...
def generate_additional_negatives(queries_dataset, document_index, number_of_negatives_added_ratio: float, lower_bound_for_negatives: int):
    ...
    for i in tqdm(range(len(queries_dataset_copy))):
        ...
        random_negative_sample = random.randint(lower_bound_for_negatives, max(99, len(samples) - 1))
...
def generate_additional_positives(queries_dataset, document_index, number_of_positives_added_ratio: float, score_threshold: float):
    ...
    for i in tqdm(range(len(queries_dataset_copy))):
        ...
        for j in range(1, len(scores)):
            if scores[j] - scores[0] <= score_threshold and scores[0] * 1.05 > scores[j]: #and scores[0] > 0.2
                positive_queries.append(question)
                positive_documents.append(samples["document"][j])
                positive_labels.append("Yes")
